...
package com.ab.hongyang.percent;
...
public class PercentLayoutHelper {
    ...
    private static boolean shouldHandleMeasuredWidthTooSmall(View view, PercentLayoutInfo info) {
        int state = view.getMeasuredWidthAndState() & -16777216;
        if (info == null || info.widthPercent == null) {
            return false;
        }
        return state == 16777216 && info.widthPercent.percent >= 0 &&
                info.mPreservedParams.width == ViewGroup.LayoutParams.WRAP_CONTENT;
    }

    private static boolean shouldHandleMeasuredHeightTooSmall(View view, PercentLayoutInfo info) {
        int state = view.getMeasuredHeightAndState() & -16777216;
        if (info == null || info.heightPercent == null) {
            return false;
        }
        return state == 16777216 && info.heightPercent.percent >= 0 &&

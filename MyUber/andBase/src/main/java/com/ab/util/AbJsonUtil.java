...
public class AbJsonUtil<T> {
	 ...
	 * 解析字符串为对象
	 */
	public static <T> T getBeanData(String content, Class<T> bean) {
		if (TextUtils.isEmpty(content)) {
			try {
				throw new Exception("Json 字符串为空");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
//			return JSONObject.parseObject(content, bean);
		} catch (Exception e) {
			e.printStackTrace();
			try {
    ...
    public static void main(String[] args) {
    	User user = getBeanData("[{id:1,name:22},{id:2,name:33}]", User.class);
